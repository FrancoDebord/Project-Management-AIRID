...
<head>
    ...
    <title>Amendment &amp; Deviations Checklist — {{ $inspection->inspection_name ?? $inspection->type_inspection }}</title>
    <style>
...
<div class="document">
    ...
    <div class="doc-ref-strip">
        <div class="ref-item">
            ...
            <span class="ref-val">Amendment &amp; Deviations Inspection Checklist</span>
        </div>
        <div class="ref-item">
            <span class="ref-label">Réf. :</span>
            <span class="ref-val">{{ $form['doc_ref'] }}</span>
        </div>
        <div class="ref-item">
    ...
    <table class="info-table">
        <tr>
            ...
            <td>{{ $inspection->inspection_name ?? $inspection->type_inspection }}</td>
        ...
        @endif
        <tr>
            <td class="label-col">Document Type</td>
            <td>{{ ($mode !== 'empty' && $record) ? ($record->document_type ?? '——') : '' }}</td>
            <td class="label-col">Amendment N° / Deviation N°</td>
            <td>
                @if($mode !== 'empty' && $record)
                    @if($record->amendment_number)
                        Amdt: {{ $record->amendment_number }}
                    @endif
                    @if($record->deviation_number)
                        Dev: {{ $record->deviation_number }}
                    @endif
                    @if(!$record->amendment_number && !$record->deviation_number)
                        ——
                    @endif
                @endif
            </td>
    ...
    <table class="questions-table">
        ...
        <tbody>
            @foreach ($form['questions'] as $n => $question)
                @php
                    $val = ($mode !== 'empty' && $record) ? $record->{"q{$n}"} : null;
                @endphp
                <tr>
                    <td class="q-num">{{ $n }}</td>
                    <td>{{ $question }}</td>
        ...
        <tfoot>
            <tr class="comments-row">
                <td colspan="5">
                    ...
                    @if($mode !== 'empty' && $record && $record->comments)
    ...
    @if($mode !== 'empty' && $record)
    @php $confVal = $record->is_conforming; @endphp
